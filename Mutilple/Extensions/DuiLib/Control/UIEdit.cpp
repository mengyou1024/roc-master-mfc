...
namespace DuiLib
{
	...
	void CEditUI::SetTextExt(LPCTSTR pstrText)
	{
		m_sTextExt = pstrText;
	}
	...
	std::pair<uint16_t, uint16_t> CEditUI::GetSel() {
		if (m_pWindow != NULL) {
            uint32_t sel =  Edit_GetSel(*m_pWindow);
            return std::make_pair(LOWORD(sel), HIWORD(sel));
		}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("textext")) == 0) SetTextExt(pstrValue);
