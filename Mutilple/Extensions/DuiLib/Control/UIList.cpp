...
CListTextElementUI::CListTextElementUI() : m_nLinks(0), m_nHoverLink(-1), m_pOwner(NULL)
{
	...
	::ZeroMemory(m_ItemTextColor, sizeof(m_ItemTextColor) );
}
...
void CListTextElementUI::SetItemTextColor(int iIndex, DWORD dwColor )
{
	m_ItemTextColor[iIndex] = dwColor;
}
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
	...
	DWORD iItemTextColor = iTextColor;
	for( int i = 0; i < pInfo->nColumns; i++ )
	{
		if( m_ItemTextColor[i] != 0 )
			iItemTextColor = m_ItemTextColor[i];
		else
			iItemTextColor = iTextColor;
		...
		if( pInfo->bShowHtml )
			CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem, strText.GetData(), iItemTextColor, \
			&m_rcLinks[m_nLinks], &m_sLinks[m_nLinks], nLinks, DT_SINGLELINE | pInfo->uTextStyle);
		else
			CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.GetData(), iItemTextColor, \
			pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
