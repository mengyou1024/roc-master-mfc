...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK)
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled() ) 
			{
				...
				if( m_iClickType != 0 && event.Type == UIEVENT_BUTTONDOWN )
					m_pManager->SendNotify(this, DUI_MSGTYPE_CLICK_DOWN);
			}
			return;
		}	
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) 
			{
				...
				if( m_iClickType != 0 )
					m_pManager->SendNotify(this, DUI_MSGTYPE_CLICK_UP);
	...
	void CButtonUI::SetClickType( INT iType )
	{
		m_iClickType = iType;
	}
}