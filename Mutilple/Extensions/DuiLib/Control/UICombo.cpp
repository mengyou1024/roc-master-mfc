...
void CComboUI::EnableItemMessage(bool bEnable /*= true*/)
{
	m_bEnableItemMessage = bEnable;
}
...
bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
{
	...
	if( pControl->IsVisible() )
	{
		if( m_pWindow != NULL || bTakeFocus ) pControl->SetFocus();
		pListItem->Select(true);
		if( m_pManager != NULL && m_bEnableItemMessage ) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
		Invalidate();
	}
#endif
	return true;
