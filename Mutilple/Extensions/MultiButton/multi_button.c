...
/**
  ...
  * @retval None
  */
void button_init(struct Button* handle, uint8_t(*pin_level)(uint8_t), uint8_t active_level, uint8_t button_id, void *userData)
{
	...
	case 2:
		if(handle->button_level == handle->active_level) { //press down again
			...
			if(handle->repeat == 1) {
				...
				handle->event = (uint8_t)MDOUBLE_CLICK;
				EVENT_CB(MDOUBLE_CLICK); // repeat hit
			}
			handle->state = 0;
		}
		break;
