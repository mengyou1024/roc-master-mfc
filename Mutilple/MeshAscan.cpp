...
void MeshAscan::UpdateData(float* pEchos, size_t iAScanSize) {
    if (m_pAscanVertices.size() < iAScanSize) {
        m_pAscanVertices.resize(iAScanSize);
        glBindBuffer(GL_ARRAY_BUFFER, m_iAscanVBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(PT_V2F_C4F) * iAScanSize, NULL, GL_DYNAMIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }

    for (size_t i = 0; i < iAScanSize; i++) {
        m_pAscanVertices[i].x = float(i) / float(iAScanSize - 1);
        m_pAscanVertices[i].y = pEchos[i];
    ...
    m_iAScanSize = (GLsizei)iAScanSize;
...
void MeshAscan::UpdateDac(int iDac, float* pDacLine, size_t iAScanSize) {
    m_pDACLineVertices[iDac].resize(iAScanSize);
    glBindBuffer(GL_ARRAY_BUFFER, m_iDACLineVBO[iDac]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(PT_V2F_C4F) * iAScanSize, NULL, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    for (int i = 0; i < iAScanSize; i++) {
        m_pDACLineVertices[iDac][i].x = float(i) / float(iAScanSize - 1);
        m_pDACLineVertices[iDac][i].y = pDacLine[i];
        m_pDACLineVertices[iDac][i].a = 1.0f;
        m_pDACLineVertices[iDac][i].r = 0.0f;
        m_pDACLineVertices[iDac][i].g = 1.0f;
        m_pDACLineVertices[iDac][i].b = 0.0f;
    }
    m_iDACLineSize[iDac] = (GLsizei)iAScanSize;

    glBindBuffer(GL_ARRAY_BUFFER, m_iDACLineVBO[iDac]);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(PT_V2F_C4F) * m_pDACLineVertices[iDac].size(), &m_pDACLineVertices[iDac][0]);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
void MeshAscan::UpdateDACData(float* pEchos, size_t iAScanSize) {
    if (m_pDACVertices.size() < iAScanSize) {
        m_pDACVertices.resize(iAScanSize);
        glBindBuffer(GL_ARRAY_BUFFER, m_iDACVBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(PT_V2F_C4F) * iAScanSize, NULL, GL_DYNAMIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
    ...
    for (size_t i = 0; i < iAScanSize; i++) {
        m_pDACVertices[i].x = float(i) / float(iAScanSize - 1);
        m_pDACVertices[i].y = pEchos[i];
        m_pDACVertices[i].a = 1.0f;
        m_pDACVertices[i].r = 0.0f;
        m_pDACVertices[i].g = 1.0f;
        m_pDACVertices[i].b = 0.0f;
    }

    m_iDACSize = (GLsizei)iAScanSize;
    glBindBuffer(GL_ARRAY_BUFFER, m_iDACVBO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(PT_V2F_C4F) * m_pDACVertices.size(), &m_pDACVertices[0]);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
